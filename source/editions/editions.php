...
class YellowEditions {
    const VERSION = "0.8.21.3";
    public $yellow;         // access to API

    // Handle initialisation
    public function onLoad($yellow) {
        $this->yellow = $yellow;
        $this->yellow->system->setDefault("editionsFeedLocation", "/feed/");
        $this->yellow->system->setDefault("editionsFeedTitle", "RSS feed: Catalogue");
        $this->yellow->system->setDefault("editionsFeedFileXml", "livres.xml");
        $this->yellow->system->setDefault("editionsFeedFilterLayout", "blog");
        $this->yellow->system->setDefault("editionsFeedPaginationLimit", "30");
    }
    ...
    public function onParsePageLayout($page, $name) {
        if ($name=="feed") {
            ...
            $feedFilterLayout = $this->yellow->system->get("editionsFeedFilterLayout");
            if (!empty($feedFilterLayout)) $pages->filter("layout", $feedFilterLayout);
            $chronologicalOrder = ($this->yellow->system->get("editionsFeedFilterLayout")!="blog");
            if ($this->isRequestXml($page)) {
                $pages->sort($chronologicalOrder ? "modified" : "published", false);
                $pages->limit($this->yellow->system->get("editionsFeedPaginationLimit"));
                ...
                $output .= "<atom:link href=\"" . $this->yellow->page->getUrl() . $this->yellow->lookup->normaliseArguments("page:" . $this->yellow->system->get("editionsFeedFileXml"), false) . "\" rel=\"self\" type=\"application/rss+xml\" />\r\n";
                ...
                $pages->paginate($this->yellow->system->get("editionsFeedPaginationLimit"));
    ...
    public function onParsePageExtra($page, $name) {
        $output = null;
        if ($name=="header") {
            $locationFeed = $this->yellow->system->get("coreServerBase") . $this->yellow->system->get("editionsFeedLocation");
            $locationFeed .= $this->yellow->lookup->normaliseArguments("page:" . $this->yellow->system->get("editionsFeedFileXml"), false);
            $output = "<link rel=\"alternate\" type=\"application/rss+xml\" title=\"" . $this->yellow->system->get("editionsFeedTitle") . "\" href=\"$locationFeed\" />\n";
        }
        return $output;
    }
    ...
    public function isRequestXml($page) {
        return $page->getRequest("page")==$this->yellow->system->get("editionsFeedFileXml");
    }
