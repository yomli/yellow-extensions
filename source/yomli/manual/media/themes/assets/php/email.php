...
if (!empty($_POST) && 
	...
	if ($_POST["status"] == "send") {
		...
		$to = mb_encode_mimeheader("Éditions Yomli")." <admin@test.com>";
		$subject = mb_encode_mimeheader("[Éditions Yomli] Message");
		...
		if ($spamFilter!="none" && preg_match("/$spamFilter/i", $message)) {
			$subject = mb_encode_mimeheader("[SPAM] [Éditions Yomli] Message");
